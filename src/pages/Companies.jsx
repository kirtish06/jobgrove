    ...
    <main className="max-w-[1200px] mx-auto px-6 py-16">
      <div className="text-center mb-10">
        ...
        <p className="text-on-surface-muted">
          Discover great places to work. {companiesData.length} companies hiring right now.
        </p>
      </div>
      ...
      {filteredCompanies.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredCompanies.map((company) => {
            ...
            return (
              <div key={company.id} className="bg-white rounded-xl border border-outline-light soft-shadow hover-lift p-6 flex flex-col">
                ...
                <p className="text-on-surface-muted text-sm mb-6 flex-1">{company.description}</p>
                <div className="flex items-center justify-between pt-4 border-t border-outline-light">
                  <div className="flex items-center gap-1 text-primary text-xs font-semibold">
                    <span className="material-symbols-outlined text-[16px]">work</span>
                    {companyJobs.length > 0 ? `${companyJobs.length} open role${companyJobs.length > 1 ? "s" : ""}` : "No open roles"}
                  </div>
